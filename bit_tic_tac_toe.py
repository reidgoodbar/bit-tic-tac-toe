...
def check_rows(board):
    for j in [1,2]:
        for i in range(0,3):
            if (((board >> 6*i & 3) == j) and ((board >> 2+(6*i) & 3) == j) and ((board >> 4+(6*i) & 3) == j)):
                return j
...
def check_cols(board):
    for j in [1,2]:
        for i in range(0,3):
            if (((board >> 2*i & 3) == j) and ((board >> 6+(2*i) & 3) == j) and ((board >> 12+(2*i) & 3) == j)):
                return j

# check_diag: determines if any diag is complete:
def check_diag(board):
    for i in [1,2]:
        if (((board & 3) == i) and ((board >> 8 & 3) == i) and ((board >> 16 & 3) == i)) or (((board >> 4 & 3) == i) and ((board >> 8 & 3) == i) and ((board >> 12 & 3) == i)):
                return i
...
def game_over(board):
    return check_rows(board) or check_cols(board) or check_diag(board) or check_tie(board)
